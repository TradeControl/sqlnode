CREATE TABLE [Invoice].[tbInvoice] (
    ...
    CONSTRAINT [FK_Invoice_tbInvoice_ParentSubjectCode] FOREIGN KEY ([ParentSubjectCode]) REFERENCES [Subject].[tbSubject] ([SubjectCode]),
...
AS
    ...
    BEGIN TRY
        ...
        (
            ...
            WHERE Subject.tbSubject.TransmitStatusCode > 1
        )
        BEGIN
...
AS
    ...
    BEGIN TRY
        ...
        (
            SELECT
                ...
                ParentCount = (
                    ...
                    WHERE tbNamespace.ChildSubjectCode = invoice.SubjectCode
                ),
            ...
            FROM Invoice.tbInvoice invoice
                JOIN inserted i ON invoice.InvoiceNumber = i.InvoiceNumber
        )
        UPDATE invoice
        SET ParentSubjectCode = namespace_candidates.SingleParentSubjectCode
        FROM Invoice.tbInvoice invoice
            JOIN namespace_candidates ON invoice.InvoiceNumber = namespace_candidates.InvoiceNumber
        ...
        (
            ...
            FROM Invoice.tbInvoice invoice
                JOIN inserted i ON invoice.InvoiceNumber = i.InvoiceNumber
            WHERE invoice.ParentSubjectCode IS NULL
              AND (
                    SELECT COUNT(*)
                    FROM Subject.tbNamespace tbNamespace
                    WHERE tbNamespace.ChildSubjectCode = invoice.SubjectCode
        ...
        (
            ...
            FROM Invoice.tbInvoice invoice
                JOIN inserted i ON invoice.InvoiceNumber = i.InvoiceNumber
            WHERE invoice.ParentSubjectCode IS NOT NULL
              AND NOT EXISTS
              (
                  SELECT 1
                  FROM Subject.tbNamespace tbNamespace
                  WHERE tbNamespace.ChildSubjectCode = invoice.SubjectCode
                    AND tbNamespace.ParentSubjectCode = invoice.ParentSubjectCode
              )
        )
        BEGIN
        ...
        BEGIN
            ...
            FROM inserted i
                INNER JOIN App.tbDocSpool ON i.InvoiceNumber = App.tbDocSpool.DocumentNumber
        ...
        (
            SELECT *
            FROM inserted
                JOIN deleted ON inserted.InvoiceNumber = deleted.InvoiceNumber AND inserted.DueOn = deleted.DueOn
        )
        BEGIN
            UPDATE invoice
            SET DueOn = App.fnAdjustToCalendar(
                    CASE
        ...
        (
            SELECT *
            FROM inserted
                JOIN deleted ON inserted.InvoiceNumber = deleted.InvoiceNumber AND inserted.ExpectedOn = deleted.ExpectedOn
        )
        BEGIN
            UPDATE invoice
            SET ExpectedOn = App.fnAdjustToCalendar(
                    CASE
                        WHEN Subject.PayDaysFromMonthEnd <> 0 THEN
        ...
        WITH invoices AS
        (
            SELECT inserted.InvoiceNumber, inserted.SubjectCode, inserted.InvoiceStatusCode, inserted.DueOn, inserted.InvoiceValue, inserted.TaxValue, inserted.PaidValue, inserted.PaidTaxValue
            FROM inserted
                JOIN deleted ON inserted.InvoiceNumber = deleted.InvoiceNumber
            WHERE inserted.InvoiceStatusCode = 1
              AND deleted.InvoiceStatusCode = 0
        )
        ...
        FROM invoices
            JOIN Subject.tbSubject Subjects ON invoices.SubjectCode = Subjects.SubjectCode;
        ...
        BEGIN
            ...
            (
                ...
                FROM Invoice.tbChangeLog clog
                    JOIN candidates ON clog.InvoiceNumber = candidates.InvoiceNumber
                                   AND LogId = (SELECT MAX(LogId) FROM Invoice.tbChangeLog WHERE InvoiceNumber = candidates.InvoiceNumber)
            )
            ...
            SELECT candidates.InvoiceNumber,
                   CASE Subjects.TransmitStatusCode WHEN 1 THEN 2 ELSE 0 END TransmitStatusCode,
                   candidates.InvoiceStatusCode,
                   candidates.DueOn,
                   candidates.InvoiceValue,
                   candidates.TaxValue,
                   candidates.PaidValue,
                   candidates.PaidTaxValue
            FROM candidates
...
AS
    ...
    BEGIN TRY
        ...
        SET DueOn = App.fnAdjustToCalendar(
                CASE
                    ...
                    ELSE
                        DATEADD(d, Subject.PaymentDays, invoice.InvoicedOn)
                END, 0),
            ExpectedOn = App.fnAdjustToCalendar(
                CASE
                    WHEN Subject.PayDaysFromMonthEnd <> 0 THEN
                        DATEADD(d, -1, DATEADD(m, 1, CONCAT(FORMAT(DATEADD(d, Subject.PaymentDays + Subject.ExpectedDays, i.InvoicedOn), 'yyyyMM'), '01')))
                    ELSE
        ...
        SELECT inserted.InvoiceNumber,
               Subject.tbSubject.TransmitStatusCode,
               inserted.InvoiceStatusCode,
               inserted.DueOn,
               inserted.InvoiceValue,
               inserted.TaxValue
        FROM inserted
            INNER JOIN Subject.tbSubject ON inserted.SubjectCode = Subject.tbSubject.SubjectCode
