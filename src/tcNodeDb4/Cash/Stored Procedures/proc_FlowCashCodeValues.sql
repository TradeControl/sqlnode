...
AS
	...
	BEGIN TRY
		...
		FROM Cash.tbPeriod
			INNER JOIN App.tbYearPeriod ON Cash.tbPeriod.StartOn = App.tbYearPeriod.StartOn
		...
		BEGIN
			...
			FROM @tbReturn cashcode_values
				JOIN active_periods ON cashcode_values.StartOn = active_periods.StartOn;
		...
		BEGIN
			...
			(
				SELECT
					Project.ProjectCode,
					(SELECT TOP (1) StartOn FROM App.tbYearPeriod WHERE (StartOn <= Project.ActionOn) ORDER BY StartOn DESC) AS StartOn,
					Project.TotalCharge,
					ISNULL(tax.TaxRate, 0) AS TaxRate
				FROM Project.tbProject AS Project
					INNER JOIN App.tbTaxCode AS tax ON Project.TaxCode = tax.TaxCode
			...
			(
				...
				FROM Projects_foryear
					LEFT OUTER JOIN order_invoice_value ON Projects_foryear.ProjectCode = order_invoice_value.ProjectCode
			), order_summary AS
			(
				SELECT StartOn, SUM(InvoiceValue) AS InvoiceValue, SUM(InvoiceTax) AS InvoiceTax
				FROM orders
			...
			FROM @tbReturn cashcode_values
				JOIN order_summary ON cashcode_values.StartOn = order_summary.StartOn;
		END
